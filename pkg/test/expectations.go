...
func ExpectCleanedUpOld(ctx context.Context, c client.Client) {
	GinkgoHelper()
	wg := sync.WaitGroup{}
	namespaces := &v1.NamespaceList{}
	Expect(c.List(ctx, namespaces)).To(Succeed())
	ExpectFinalizersRemovedFromList(ctx, c, &v1beta1.NodeClaimList{})
...
func ExpectCleanedUp(ctx context.Context, c client.Client) {
	...
	for _, object := range []client.Object{
		...
		go func(obj client.Object) {
			...
			Expect(c.List(ctx, metaList, client.HasLabels([]string{test.DiscoveryLabel}))).To(Succeed())
			...
			workqueue.ParallelizeUntil(ctx, 50, len(metaList.Items), func(i int) {
				defer GinkgoRecover()
				Eventually(func(g Gomega) {
					g.Expect(client.IgnoreNotFound(c.Delete(ctx, &metaList.Items[i], client.PropagationPolicy(metav1.DeletePropagationForeground),
						&client.DeleteAllOfOptions{DeleteOptions: client.DeleteOptions{GracePeriodSeconds: ptr.Int64(0)}}))).To(Succeed())
			...
			Eventually(func(g Gomega) {
				metaList = &metav1.PartialObjectMetadataList{}
				metaList.SetGroupVersionKind(lo.Must(apiutil.GVKForObject(obj, c.Scheme())))
				g.Expect(c.List(ctx, metaList, client.HasLabels([]string{test.DiscoveryLabel}))).To(Succeed())
				//g.Expect(len(metaList.Items)).To(BeZero())
			}).WithPolling(time.Second * 30).Should(Succeed())
		}(object)
	}
	wg.Wait()
}
