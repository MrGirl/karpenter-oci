...
var _ = Describe("SecurityGroups", func() {
	It("should use the security-group-id selector", func() {
		securityGroups := env.GetSecurityGroups("vcnid", []string{"test-sg"})
		...
		Expect(env.GetSecurityGroupByInstanceId(pod.Spec.NodeName)).To(Equal(lo.Map(securityGroups, func(sg core.NetworkSecurityGroup, _ int) string { return lo.FromPtr(sg.Id) })))
	})
