...
var _ = Describe("CNITests", func() {
	It("should set eni-limited maxPods", func() {
		...
		Expect(allocatablePods).To(Equal(eniLimitedPodsFor(node.Labels["node.kubernetes.io/instance-type"], vcpu/2)))
	})
...
func eniLimitedPodsFor(instanceType string, ocpu int) int64 {
	shapes, err := env.CMPAPI.ListShapes(env.Context, core.ListShapesRequest{
		CompartmentId:      lo.ToPtr("shape_comp"),
		AvailabilityDomain: lo.ToPtr("AD"),
	})
	Expect(err).ToNot(HaveOccurred())
