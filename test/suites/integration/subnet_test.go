...
var _ = Describe("Subnets", func() {
	It("should use the subnet-id selector", func() {
		subnets := env.GetSubnets("vcnid", []string{"subnet"})
		...
		Expect(env.GetSubnetByInstanceId(pod.Spec.NodeName)).To(Equal(lo.FromPtr(subnets[0].Id)))
	})
	It("should use the subnet tag selector with name", func() {
		...
		Expect(env.GetSubnetByInstanceId(pod.Spec.NodeName)).To(Equal(lo.FromPtr(firstSubnet.Id)))
	})
